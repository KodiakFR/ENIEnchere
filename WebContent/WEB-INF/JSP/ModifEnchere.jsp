			...
			<form action="ModifEnchere" method="post" id="formNouvelleVente">
					 ...
					 <div class="form-group row">
		   				 <label for="nomArticle" class="col-sm-2 col-form-label">Article</label>
			   				 <div class="col-sm-10">
			     				 <input type="text" class="form-control" id="nomArticle" name="nomArticle" maxlength="30" required="required" disabled="disabled" value="${articleAModifier.nomArticle }">
			    			</div>
		  			</div>
		  			...
		  			<div class="form-group row">
		   				 <label for="categorie" class="col-sm-2 col-form-label">Categorie</label>
								...
								<select class="custom-select" id="categorie" name="categorie">
									<option value="${articleAModifier.categorie}" autofocus="autofocus">${articleAModifier.categorie}</option>
								    <c:forEach var="cat" items="${listeDeCategories}">
								    	<c:if test="${cat.libelle != articleAModifier.categorie}">	
...
<!-- 		  			Affichage de l'image si il y en a une -->
				...
				<input type="button" value="Enregistrer" class="btn btn-primary mb-2">
