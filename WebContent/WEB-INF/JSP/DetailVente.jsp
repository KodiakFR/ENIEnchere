	...
	<c:if test="${vendeur != Utilisateur}">
	        <div class="form-group row">
	            <label for="map" class="col-sm-4 col-form-label">Ma Proposotion</label>
	           	<div class="col-sm-6">
	                     <input type="number" <c:if test="${empty enchere }"> min="${articleAAfficher.miseAPrix+10}" value="${articleAAfficher.miseAPrix+10}"</c:if><c:if test="${!empty enchere }"> min="${enchere.getMontantEnchere()+10}" value="${enchere.getMontantEnchere()+10}"</c:if> name="enchere"/>
